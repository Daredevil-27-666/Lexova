...
// ─── standalone sub-components (module level to prevent remounting) ───────────
function GoogleIcon() {
  return (
...
function AuthSheet({
  authMode, email, password, showPw, loading, oauthLoading,
  error, successMsg, onEmailChange, onPasswordChange,
  onToggleShowPw, onGoogleOAuth, onSubmit, onToggleMode, onClose,
}: AuthSheetProps) {
  return (
    <div
    ...
    >
      ...
      >
        ...
        <p className="text-center font-['Inter'] text-xs pb-2" style={{ color: '#475569' }}>
          ...
          <button onClick={onToggleMode} style={{ color: '#3B82F6' }}>
...
export function Onboarding() {
  ...
  const closeAuth = useCallback(() => setAuthMode('none'), []);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden" style={{ backgroundColor: '#0A0A0B' }}>
      ...
      <div
        ...
        style={{ width: '400%', transform: `translateX(-${(slide / 4) * 100}%)` }}
      >
        {/* ── Slide 0: Welcome ── */}
        <div className="h-full relative flex flex-col items-center justify-center overflow-hidden" style={{ width: '25%', flexShrink: 0 }}>
        ...
        <div className="h-full relative flex overflow-hidden" style={{ width: '25%', flexShrink: 0, backgroundColor: '#0A0A0B' }}>
        ...
        {/* ── Slide 2: The Hook ── */}
        <div className="h-full relative flex flex-col justify-end overflow-hidden" style={{ width: '25%', flexShrink: 0, backgroundColor: '#0A0A0B' }}>
          <div className="absolute inset-0">
        ...
        {/* ── Slide 3: Auth ── */}
        <div
          className="h-full relative flex flex-col justify-between overflow-hidden"
          style={{ width: '25%', flexShrink: 0, background: 'linear-gradient(to bottom, #1a1025, #0A0A0B)' }}
        >
      ...
      {/* auth sheet overlay — stable component, no remounting on state changes */}
      {authMode !== 'none' && (
        <AuthSheet
          authMode={authMode as Exclude<AuthMode, 'none'>}
          email={email}
          password={password}
          showPw={showPw}
          loading={loading}
