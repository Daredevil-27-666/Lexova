...
export function ArtistPanel({ artistId = '' }: ArtistPanelProps) {
  ...
  return (
    ...
    >
      {/* Artist Header */}
      <div className="px-4 sm:px-6 pt-5 sm:pt-6 pb-5 border-b" style={{ borderColor: '#1a1a1a' }}>
        <div className="flex items-start gap-4 sm:gap-6">
          <div className="flex-shrink-0">
            <img
              src={artist.avatarUrl}
              alt={artist.name}
              className="w-20 h-20 sm:w-28 sm:h-28 rounded-full object-cover ring-2 ring-[var(--gold-accent)]"
            />
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <h2
              className="font-['Playfair_Display'] mb-2 sm:mb-3 text-2xl sm:text-[2rem] leading-tight font-semibold truncate"
            ...
            <div className="flex items-center gap-2 mb-2 sm:mb-3 flex-wrap">
              {artist.genres.map((genre) => (
                <span
                  key={genre}
                  className="px-2.5 py-0.5 rounded-full font-['DM_Sans'] text-xs"
                  style={{
            ...
            <p
              className="font-['DM_Sans'] mb-3 line-clamp-3 text-sm sm:text-[0.9375rem] leading-relaxed hidden sm:block"
              style={{ color: 'var(--text-secondary)' }}
            >
              {artist.bio}
            </p>
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <div>
                <div
                  className="font-['DM_Sans'] text-xl sm:text-2xl font-semibold leading-none"
                  style={{ color: 'var(--gold-accent)' }}
                >
      ...
      <div className="border-b" style={{ borderColor: '#1a1a1a' }}>
        <div role="tablist" className="flex items-center gap-0 px-4 sm:px-6 overflow-x-auto scrollbar-none">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              role="tab"
              ...
              className="px-3 sm:px-4 py-3 font-['DM_Sans'] text-sm transition-all relative flex-shrink-0"
              style={{
                color:
                  activeSubTab === tab.id
                    ? 'var(--text-primary)'
